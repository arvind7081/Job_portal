	...
	$.ajax({
		...
		success: function(response){
			if(response.status === false){
				...
				// ✅ SUCCESS CASE
				$("#image-error").html(''); // Clear error
				$('#image').val(''); // Clear file input
				$('#exampleModal').modal('hide'); // Hide modal (Bootstrap 5)
				
				// Optional: Refresh the page to show new image
				location.reload(); // OR use AJAX to update image dynamically
			}
		},
		error: function(xhr){
